  ...
  try {
    switch (method) {
      case "GET":
        if (id) {
          // Get single story
          const story = await StoryAPI.getById(id as string);
          if (!story) {
      ...
      case "PUT":
        ...
        const updatedStory = await StoryAPI.update(id as string, req.body);
        if (!updatedStory) {
          return res.status(404).json({ error: "Story not found" });
        }
      ...
      case "DELETE":
        ...
        const deleted = await StoryAPI.delete(id as string);
        if (!deleted) {
          return res.status(404).json({ error: "Story not found" });
        }
